...
contract SpaceBar is Ownable, ReentrancyGuard, Pausable, AccessControl {
    ...
    function claim() external whenNotPaused {
        ...
        users[msg.sender] = User({balance: initialBalance, highScore: 0, claimed: true});
    ...
    function playMatch(uint256 matchId, uint256 player1Score, uint256 player2Score) external nonReentrant {
        ...
        if (player1Score > player2Score) {
            ...
            users[currentMatch.player1].balance =
                users[currentMatch.player1].balance.add(currentMatch.player1Stake).add(currentMatch.player2Stake);
        } else if (player2Score > player1Score) {
            winner = currentMatch.player2;
            users[currentMatch.player2].balance =
                users[currentMatch.player2].balance.add(currentMatch.player1Stake).add(currentMatch.player2Stake);
        }
